...
class ShuiRuntimeController private constructor(
    ...
    override fun refreshShower798Devices() {
        ...
        runShower798Action(
            ...
            saveLocalDevices(localDevices)
            postState {
                val selected = state.currentShower798DeviceId.takeIf { id -> devices.any { it.id == id } }
                    ?: devices.firstOrNull()?.id.orEmpty()
                state = state.copy(
                    shower798Devices = devices,
    ...
    override fun selectShower798Device(deviceId: String) {
        val nextId = if (state.currentShower798DeviceId == deviceId) "" else deviceId
        state = state.copy(currentShower798DeviceId = nextId)
    }
